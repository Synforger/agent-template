...
EXCLUDE_DIRS = {".git", "node_modules", "dist", "build", ".venv", "venv",
                ...
                # `task init` promotes them to the repo root. For this
                # template the payload is `src/`; its docs are checked by
                # the shipped docs-check inside a derived agent.
                "_core", "src"}
# The template's own docs describe the derived layout (= what `src/`
# becomes after init), so a path that resolves under the payload root
# counts as live.
PAYLOAD_ROOT = "src"
...
def path_exists(cand, md_dir):
    ...
    return (os.path.exists(os.path.join(ROOT, cand))
            or os.path.exists(os.path.join(ROOT, PAYLOAD_ROOT, cand))
